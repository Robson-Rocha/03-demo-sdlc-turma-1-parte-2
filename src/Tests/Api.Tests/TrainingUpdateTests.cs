...
public sealed class TrainingUpdateTests
{
    ...
    public async Task ReturnsBadRequestWhenRequestIsInvalid()
    {
        using var factory = new TrainingCatalogApiFactory();
        using var client = factory.CreateClient();
        var request = new CreateTrainingRequest("", "Descrição", "2026-09-15", 8, 0, 0);
    ...
    [Fact]
    public async Task ReturnsBadRequestWhenLessonPlanExceedsTrainingDuration()
    {
        using var factory = new TrainingCatalogApiFactory();
        using var client = factory.CreateClient();
        var createdTraining = await CreateTraining(client, "2026-09-15");
        var request = new CreateTrainingRequest("C# Avançado", "Tópicos avançados de C#", "2026-09-16", 8, 3, 4);
